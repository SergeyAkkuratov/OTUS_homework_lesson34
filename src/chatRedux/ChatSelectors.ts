...
export function selectMessagesFromAuthor(state: ChatState, nickname: string): ChatMessage[] {
    return state.messages.filter((message) => message.nickname === nickname);
}

export function selectMessagesAfterDate(state: ChatState, date: Date): ChatMessage[] {
